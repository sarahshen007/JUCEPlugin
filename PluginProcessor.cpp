...
void Homework4AudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    // A temporary buffer
    juce::AudioBuffer<float> temporaryBuffer(buffer.getNumChannels(), numberSamples);
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        // A temporary writer to this channel of the temporary buffer
        auto* temporaryChannelData = temporaryBuffer.getWritePointer(channel);

        // to change the gain, from 0.0f to 1.0f
        for (int sample = 0; sample < buffer.getNumSamples(); ++sample) {
            channelData[sample] = channelData[sample] * mGain;
        }

        int sampleInterval = 1 + mPitch;
        int bufferSize = buffer.getNumSamples() / sampleInterval;

        for (int sample = 0; sample < buffer.getNumSamples(); ++sample) {
            if (sample % sampleInterval == 0) {
                temporaryChannelData[sample / sampleInterval] = channelData[sample];
            }
        }

        float prevVal = temporaryChannelData[0];
        for (int sample = 0; sample < buffer.getNumSamples(); ++sample) {
            if (sample % sampleInterval == 0) {
                channelData[sample] = temporaryChannelData[sample % bufferSize];
                prevVal = channelData[sample];
            }
            else {
                channelData[sample] = prevVal;
            }
        }
    }
}
